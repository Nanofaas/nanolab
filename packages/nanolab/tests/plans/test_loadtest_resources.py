...
def test_concurrency_uses_the_calibrated_default_when_the_scenario_says_nothing() -> None:
    config = ScenarioConfig(
        workflow="loadtest", backend="container", concurrencyControl=True,
        functions=["word-stats-java"],
    )

    assert _concurrency_function_resources(config, "word-stats-java")["limits"]["cpu"] == 4.0
...
def test_a_scenario_declared_cpu_cap_overrides_the_default() -> None:
    ...
    config = ScenarioConfig(
        workflow="loadtest", backend="container", concurrencyControl=True,
        functions=["word-stats-java"],
        resources={"word-stats-java": {"limits": {"cpu": 2, "memoryMiB": 512},
...
def test_an_unlisted_function_keeps_the_default() -> None:
    ...
    assert _concurrency_function_resources(config, "word-stats-java-lite")["limits"]["cpu"] == 4.0
