...
from nanolab.tasks.telegram import (
...
def test_telegram_observer_sends_success_with_workflow_scenario_and_duration() -> None:
    ...
    TelegramWorkflowObserver(
        scenario="deployment-lifecycle-k8s.yaml", send=messages.append
    ).finished(_completion())
...
def test_telegram_observer_sends_failure_without_masking_transport_error() -> None:
    ...
    TelegramWorkflowObserver(
        scenario="deployment-lifecycle-k8s.yaml", send=messages.append
    ).finished(_completion(RuntimeError("control plane unavailable")))
...
def test_telegram_observer_ignores_telegram_errors() -> None:
    ...
    TelegramWorkflowObserver(scenario="scenario.yaml", send=fail_to_send).finished(
        _completion()
    )
