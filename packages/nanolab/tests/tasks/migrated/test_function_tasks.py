...
import nanolab.tasks.http_function as http_function
from nanolab.tasks.cli_function import CliFunctionApplyTask, CliFunctionDeleteTask
...
def test_cli_delete_accepts_only_a_clean_exit() -> None:
    ...
    assert executor.seen[0].options.expected_exit_codes == frozenset({0})
...
def test_http_delete_tolerates_an_unreachable_or_absent_target() -> None:
    ...
    assert executor.seen[0].options.expected_exit_codes == frozenset({0, 7, 22})
