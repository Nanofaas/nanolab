...
def test_bootstrap_tasks_run_between_build_and_the_first_resource() -> None:
    ...
    bootstrap = (
        CommandTask(
            ...
            executor=executor,
            role="host",
            semantic_key="test:provision-base",
        ),
        CommandTask(
            ...
            executor=executor,
            role="host",
            semantic_key="test:sync-repository",
        ),
    )
    workflow = build_cli_workflow(
...
def test_scaling_fails_when_the_desired_count_did_not_change() -> None:
    executor = ScriptedExecutor(
        responses={
            "fn replicas get": _passed(
                '{"name":"word-stats-java","desiredReplicas":1,"readyReplicas":1}'
            )
        }
    )
    workflow = build_cli_workflow(
...
def test_a_control_plane_missing_an_optional_capability_fails_early() -> None:
    executor = ScriptedExecutor(
        responses={
            "control-plane info": _passed(
                '{"capabilities":{"functionUpdate":true,"replicas":false}}'
            )
        }
    )
    workflow = build_cli_workflow(
