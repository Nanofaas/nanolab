"""Tests for the release composite builders in ``nanolab.tasks.release_composites``."""
...
class TestSourceTestsComposite:
    ...
    def test_preserves_command_execution_fields(self) -> None:
        ...
        assert dict(seen.options.env) == {"CI": "true"}
        assert seen.options.cwd == Path("/local/fallback")
        assert seen.options.remote_dir == "/remote/source"
        assert seen.options.expected_exit_codes == frozenset({0, 5})
        assert seen.options.timeout_seconds == 600
