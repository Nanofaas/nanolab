...
def test_an_ssh_that_dies_reports_its_own_error() -> None:
    """Report the SSH failure itself when a tunnel never comes up.

    It must not surface later as a Prometheus timeout three tasks away.
    """
    process = _FakeProcess(exits_with=255, stderr=b"bind: Address already in use")

    with (
        pytest.raises(RuntimeError, match="Address already in use"),
        prometheus_over_ssh(
            _multipass(),
            "http://192.168.64.3:30090",
            spawn=_spawn([], process),
...
def test_a_tunnel_that_never_answers_times_out() -> None:
    ...
    with (
        pytest.raises(RuntimeError, match="never accepted a connection"),
        prometheus_over_ssh(
            _multipass(),
            "http://192.168.64.3:30090",
            spawn=_spawn([], process),
