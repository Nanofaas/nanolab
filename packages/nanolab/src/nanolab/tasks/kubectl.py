"""Kubernetes task helpers shared with sonata, plus nanolab's own waits."""
...
def k8s_function_resources_absent(
    *,
    function: str,
    namespace: str,
    ...
    """Wait for a function's Deployment and Service to disappear from `namespace`.
    ...
    missing = (
        f"function {function} resources did not disappear within {timeout_seconds}s"
    )
    script = (
        f"for _ in $(seq 1 {attempts}); do present=0; {checks} "
        '[ "$present" -eq 0 ] && exit 0; sleep 2; done; '
        f"echo {shlex.quote(missing)} >&2; "
        "exit 1"
    )
    return CommandTask(
        title=f"Wait for {function} resources to disappear",
        argv=("bash", "-lc", script),
        executor=executor,
        role=role,
