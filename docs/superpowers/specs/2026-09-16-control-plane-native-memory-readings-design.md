...
- No Native Memory Tracking. It needs a JVM startup flag and a new diagnostic
  ...
  soak diagnostic operations `native_memory`, `native_memory_baseline` and
  `native_memory_diff`, for scenarios that set `-XX:NativeMemoryTracking` — not
  as an additional reading of this workflow. The pair is what makes it a
  measurement rather than a snapshot: NMT takes no baseline of its own, so the
  mark is taken at the baseline checkpoint and the diff read at drain reports
  growth per category over steady and drain.
...
Native Memory Tracking has since landed as the soak diagnostic operations
`native_memory`, `native_memory_baseline` and `native_memory_diff`, which is
where a scenario's `-XX:NativeMemoryTracking` flag now produces evidence, and
`baseline_operations` is where a reading is declared for the baseline checkpoint.
The `MALLOC_ARENA_MAX` A/B spike, and extending these readings to the soak path,
remain separate sub-projects. Choose follow-up work
